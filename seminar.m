...
% income=normrnd(1.156,0.485,[numhh,1]); %$1000
% hsize=normrnd(18.337,5.288,[numhh,1]); %100sqft
% lawnsize=normrnd(9.877,3.383,[numhh,1]); %1000sqft
% bath=normrnd(1.635,0.52,[numhh,1]);
...
p11 = 1.1:0.01:1.3;
numIter = numel(p11);
qtydIBP = zeros(numhh,numIter);
expIBP = zeros(numhh,numIter);
...
for i = 1:numIter;
    ...
    d2 = 0.001*(p12-p11(i))*x11;
    pricematrix = [p11(i)*ones(numhh,1) p11(i)*ones(numhh,1) p12*ones(numhh,1)];
    d.p11 = p11(i).^coef.p1;
    d.y2 = (income+d2).^coef.yd;
    condd1 = d.hh.*d.p11(i).*d.y1;
    ...
    expIBP(:,i) = sum(hhBlock.*condd.*pricematrix,2);
    
end

%make rows the different simulations and columns income quintiles
